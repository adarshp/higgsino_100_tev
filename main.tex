...
\abstract{% 
  ...
  TeV can be excluded for Binos with masses of about 1.3 TeV at 95\% C.L.   This search
  channel extends the multi-lepton search limits, especially in the region
  where the mass difference between Higgsino NLSP and Bino LSP is smaller.
\Shufang{Update the numbers.} \Adarsh{They are consistent with the text now.}}
...
to your comment ... let me know if I should add more details.}. \Shufang{What
you said below is for well separated spectra, not for mass spitting of 0.1 to
50 GeV.}
...
\begin{figure}[h]
  ...
  \caption{Higgsino pair production $\widetilde\chi_2^0\widetilde\chi_3^0$
    cross section (left panel), calculated using
    Prospino2~\cite{Beenakker:1999xh},  and decay decay branching fractions
    ...
    on $M_1$ is very weak. \Shufang{How about Br? What's the vcalue of M1
    used? Could you make the two plot same aspect ratios? Using plot
digitizer. Also, for ylabel in left panel, the tilde should not go above 0.
Just above $\chi$.  Is there way to change the xlabel of right panel to be
$\mu$ instead of $|\mu|$?  Need to be consistent with the xlabels.
}\Adarsh{The value of $M_1$ used is 25 GeV for the branching ratio plot as
well. I was able to rerun prospino and susyhit, so I didn't need to run the
plotdigitizer for these plots. I also changed the tildes as you suggested.
I changed the x-axis of both plots to $|\mu|$ for consistency - is that ok?}
}
...
and an equal mixture of $WZ+WH$ topology, with the most stringest limits from
...
\cite{Aaboud:2018sua,Aaboud:2018jiw, ATLAS:2016uwq,Khachatryan:2014qwa,Chatrchyan:2012pka}, assuming a
...
excluded for massless LSPs \cite{Aaboud:2018sua}.
...
%in the final state have been studied in \cite{ATLAS:2016ety,
%Khachatryan:2016trj}, with the greatest reach obtained by ATLAS in
%\cite{ATLAS:2016ety}, using 13 TeV data - they are able to exclude wino-like
...
in \cite{Aad:2014vma,Khachatryan:2015kxa,Khachatryan:2014mma}. Winos up to 465
GeV are excluded in the $2l + 0/2j$ final states in \cite{Aad:2014vma}. A
compressed mass spectrum is studied in \cite{Khachatryan:2015kxa}, with a mass
...
analysis in \cite{Khachatryan:2014mma}, where higgsinos can be excluded up to a
...
well. \Shufang{This is only for BG, right?  No such cuts are imposed for
signal generation?} \Adarsh{Yes, the MET cut was only for background.}
...
\end{align}
 for $E_{Z,h}$ and $\vec{p}_{Z,h}$ being the reconstructed energy and momentum for $Z$ and $h$, respectively.
  ...
  $m_{\ell^+\ell^-}$, and
  $\slashed{E}_{T}$ cuts.  \Shufang{How about mll cut? BG is generated with mll
  cut.}  \Adarsh{I have added the $m_{ll}$ cut to the sentence.} The
...
\begin{table}[h]
  ...
  \caption{Representative cut flow table for the benchmark point $\mu=1$ TeV,
    ...
    calculated for an integrated luminosity of 3 ab$^{-1}$. \Shufang{MET $>$
  400 or $<$ 400? } \Adarsh{$> 400$ - I've fixed it now.}}
...
\Shufang{Again, generator level MET, mll cut for the BG?} \Adarsh{Yes.} We then
...
would a regular kinematical variable, that is, we apply a cut that selects
...
\Shufang{Can we change the xlabel of all plots to $\mu$ instead of $|\mu|$ or
it is difficult to do? Did you save the data for this plot?  It is
actually not difficult to overlay ref 10 results.  They only have a few data
points and can easily be read out using plot-digitizer.  By comparison, we can
clearly see that our reach is better for large M1 region.} \Adarsh{I did not
save the data for this plot - I thought I could always regenerate it - however,
the HPC folks changed their policy on the number of files per GB that they
would allow - MadGraph and MadEvent produce a lot of files - for the Exotic
Higgs project, I came up with ways to deal with this, but to do the same for
this project would take a very large amount of time. I could change the xlabel
...
1.3 TeV. \Shufang{Cross check the numbers. Not entirely consistent with the
plot.} \Adarsh{I updated the numbers - how do they look?} The boosted decision
...
Similarly, Binos can be discovered up to 300 GeV and excluded up to 700 GeV
with the rectangular cut analysis, and the BDT analysis can discover them up to
about 900 GeV and exclude them up to 1.3 TeV. Since the background left after
...
them up to a mass of 1.3 TeV, for bino masses around 350 GeV and 700 GeV
respectively. 
...
exclude them up to a mass of 1.8 TeV, for bino masses of about 900 GeV and 1.3
TeV respectively. With the BDT analysis, Bino dark matter
candidates can be discovered up to 0.9 TeV, and excluded up to 1.3 TeV. In the
process, we highlighted the importance of generating enough Monte Carlo events
to estimate the huge backgrounds at a 100 TeV $pp$ collider.  \Shufang{Update
the numbers.} \Adarsh{They are updated now.}
